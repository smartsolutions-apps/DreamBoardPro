    ...
    try {
      ...
      for (const [index, scene] of initialScenes.entries()) {
        try {
          ...
          // 3. Upload (Guest Safe)
          const activeUserId = user ? user.uid : undefined;
...
const handleGenerateVideo = async (sceneId: string) => {
  ...
  try {
    ...
    if (true) {
      ...
      finalVideoUrl = await uploadVideoToStorage(user?.uid || 'temp_guest', projectTitle, scene.title || `scene_${safeIndex + 1}`, videoUrl);
    }
...
const handleGenerateAudio = async (sceneId: string) => {
  ...
  try {
    ...
    if (true) {
      finalAudioUrl = await uploadAudioToStorage(user?.uid || 'temp_guest', projectTitle, safeIndex, audioUrl);
    }
